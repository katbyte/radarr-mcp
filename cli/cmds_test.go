...
func run(t *testing.T, args ...string) (string, error) {
	...
	// a developer's own ~/.radarr-mcp must not leak into the run: with a
...
func TestMakeBuildsTheCommands(t *testing.T) {
	...
	out, err = run(t)
	if err != nil || !strings.Contains(out, "radarr-mcp help") {
...
func TestVersionCommand(t *testing.T) {
	...
	if err != nil || !strings.HasPrefix(out, "radarr-mcp ") {
...
func TestInfoNeedsConnectionParams(t *testing.T) {
	...
	_, err = run(t, "serve", "--server", "http://nas:7878")
	if err == nil || !strings.Contains(err.Error(), "token") {
		t.Errorf("serve with no token = %v, want a complaint about token", err)
	}
}

// `radarr-mcp tools` needs no server, so it has to work with nothing
...
func TestToolsCommand(t *testing.T) {
	...
	for _, want := range []string{"core", "server_info", "movie_list", coreTool} {
		if !strings.Contains(out, want) {
			t.Errorf("%q missing from `tools`:\n%s", want, out)
		}
	}
	if strings.Contains(out, "audit_runtime") {
...
func TestToolsCommandToolsets(t *testing.T) {
	...
	if !strings.Contains(out, "audit_runtime") {
		t.Error("audit_runtime missing from curation")
	}
	...
	if !strings.Contains(out, "movie_delete") || strings.Contains(out, "delete tools are hidden") {
