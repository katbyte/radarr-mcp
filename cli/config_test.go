...
func load(t *testing.T, home, wd string) *FlagData {
	...
	if err := configureFlags(&cobra.Command{Use: "radarr-mcp"}); err != nil {
...
func write(t *testing.T, dir, body string) {
	...
	if err := os.WriteFile(filepath.Join(dir, ".radarr-mcp"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
}
...
// A .radarr-mcp in the working directory is documented as per-project
...
func TestEnvironmentOverridesConfigFile(t *testing.T) {
	...
	t.Setenv("RADARR_SERVER", "http://from-env")
...
// Every documented RADARR_* variable has to actually reach its field; a
...
func TestEnvironmentBindings(t *testing.T) {
	...
	for _, env := range []struct {
		...
		{"RADARR_SERVER", testServer, func(f *FlagData) bool { return f.Server == testServer }},
		{"RADARR_TOKEN", "tok", func(f *FlagData) bool { return f.Token == "tok" }},
		{"RADARR_READ_ONLY", on, func(f *FlagData) bool { return f.ReadOnly }},
		{"RADARR_ENABLE_DELETE", on, func(f *FlagData) bool { return f.EnableDelete }},
		{"RADARR_LISTEN", ":8080", func(f *FlagData) bool { return f.Listen == ":8080" }},
		{"RADARR_AUTH_TOKEN", "bearer", func(f *FlagData) bool { return f.AuthToken == "bearer" }},
		{"RADARR_ALLOW_NO_AUTH", on, func(f *FlagData) bool { return f.AllowNoAuth }},
		{"RADARR_TOOLSETS", "curation", func(f *FlagData) bool { return len(f.Toolsets) == 1 && f.Toolsets[0] == "curation" }},
		{"RADARR_ALLOW_TOOLS", testTool, func(f *FlagData) bool { return len(f.AllowTools) == 1 && f.AllowTools[0] == testTool }},
		{"RADARR_DENY_TOOLS", "*_delete", func(f *FlagData) bool { return len(f.DenyTools) == 1 && f.DenyTools[0] == "*_delete" }},
	} {
...
func TestNewClientValidation(t *testing.T) {
	...
	if _, err := (&FlagData{Server: "nas:7878", Token: "t"}).NewClient(); err == nil {
