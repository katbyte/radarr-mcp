...
func TestMuxRoutes(t *testing.T) {
	...
	server := mcp.NewServer(&mcp.Implementation{Name: "radarr-mcp", Version: "test"}, nil)
...
func TestServeNeedsAnAuthToken(t *testing.T) {
	...
	} else if !strings.Contains(err.Error(), "RADARR_AUTH_TOKEN") || !strings.Contains(err.Error(), "allow-no-auth") {
