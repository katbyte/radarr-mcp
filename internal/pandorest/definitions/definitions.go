...
type Service struct {
	...
	// api-definitions directory, e.g. "radarr".
	Name string `json:"Name"`
	// Package is the Go package the generator writes, e.g. "radarr".
	...
	// "Radarr", an API key in the X-Api-Key header.
...
// other tag uses.
type Group struct {
	// Name is the Go-facing name used in file names, e.g. "MovieFile".
	Name string `json:"Name"`
	// Tag is the tag as the document spells it, e.g. "MovieFile"; empty for
	// the common group.
...
type Operation struct {
	...
	// Path is the document's path template, e.g. /api/v3/movie/{id}.
	...
	// Pageable is set for page/pageSize list operations, which also get a
	// Complete method that pages through every result.
...
// Pageable says how a list operation pages. Radarr pages by number: page
// counts from one, pageSize is the rows a page holds, and the answer carries
// the rows in records and how many there are in all in totalRecords.
...
type Field struct {
	...
	// Required is set when the document says an object must carry the
	// field: the model always sends it, empty or not.
	Required bool `json:"Required,omitempty"`
}
...
// String renders a type for logs and diffs: List[Reference(MovieResource)].
