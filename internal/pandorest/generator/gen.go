...
func newGen(svc *definitions.Service, opts Options) (*gen, error) {
	if svc.Auth != "Radarr" {
		return nil, fmt.Errorf("%s: unknown Auth %q (want Radarr)", svc.Name, svc.Auth)
	}
...
// boolean as *bool, a number the document marks nullable by pointer too (a
// null there means something - no size limit, no film - that a zero would
// say wrongly), the rest by value.
func (g *gen) fieldType(f *definitions.Field) string {
	switch f.Type.Type {
	case definitions.Boolean:
		return "*bool"
	case definitions.Integer, definitions.Integer64, definitions.Float, definitions.Double:
		if f.Nullable {
...
// list. A field the document requires is always sent, so an empty string
// goes as "" rather than not at all. The rest are omitempty.
...
func (g *gen) clientFile() string {
	body := `// Client is a client for the Radarr API; each of its operations is a method.
...
// 404.
//
// A list operation that pages with page and pageSize also has a
// <Name>Complete method that pages through every result.
//
// Dates are strings: Radarr sends RFC 3339 in UTC ("2024-03-01T00:00:00Z").
`)
