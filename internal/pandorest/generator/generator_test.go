...
import (
	...
	"github.com/katbyte/radarr-mcp/internal/pandorest/generator"
	"github.com/katbyte/radarr-mcp/internal/pandorest/importer"
...
const miniSpec = `{
  ...
  "components": {"schemas": {
    ...
    "Item": {"type": "object", "properties": {
      ...
      "Rating": {"type": "number", "format": "float"}, "Size": {"type": "integer", "format": "int64", "nullable": true},
      "Limit": {"type": "number", "format": "double", "nullable": true}, "Name": {"type": "string", "nullable": true},
...
func miniDefinitions(t *testing.T) *definitions.Service {
	...
	svc, err := importer.FromSpec(config.Service{Name: "mini", Package: "mini", Naming: config.OperationIDNaming, Auth: "Radarr"}, spec, []string{"mini-fix"}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerate(t *testing.T) {
	...
	t.Run("signatures", func(t *testing.T) {
		...
		// a nullable number is a pointer, so a null is not read as zero; a
		// nullable string stays a string; a required field is always sent
		for _, re := range []string{
			"`json:\"IsFolder,omitempty\"`", "`json:\"Children,omitzero\"`", "`json:\"Parent,omitempty\"`", `Parent +\*Item`, `Blur +\*ItemBlur`, `Rating +float32`,
			`Size +\*int64`, `Limit +\*float64`, `Name +string`, "`json:\"Name,omitempty\"`", "Path +string +`json:\"Path\"`",
		} {
