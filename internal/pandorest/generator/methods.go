...
func (g *gen) writeComplete(b *strings.Builder, o *definitions.Operation, optionsType string) {
	...
	fmt.Fprintf(b, "// %s calls %s page by page from options.%s (the first\n", method, o.Name, p.PageOption)
	...
	b.WriteString("\t\t// a short page is the last; so is reaching the total the server reports\n")
	fmt.Fprintf(b, "\t\tif len(page.Model.%[1]s) < options.%[2]s || page.Model.%[3]s > 0 && len(result.Items) >= page.Model.%[3]s {\n\t\t\treturn\n\t\t}\n",
		p.ItemsField, p.PageSizeOption, p.TotalField)
