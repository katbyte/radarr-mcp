...
// records the page asked for by each request.
func pagedServer(t *testing.T, pageParam, record string, total int) (*Client, *[]string) {
	t.Helper()
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprintf(w, "{\"records\":[%s],\"totalRecords\":%d}", record, total)
	}))
	t.Cleanup(srv.Close)
...
func TestNew(t *testing.T) {
	...
	if _, err := New("http://server:7878", ""); err == nil {
		t.Error("New without an API key succeeded")
	}
	if _, err := New("server", "test-key"); err == nil {
		t.Error("New without a scheme succeeded")
	}
	c, err := New("http://server:7878/", "test-key")
	if err != nil || c.Client.BaseURL != "http://server:7878" {
...
func (g *gen) completeTest(o *definitions.Operation, call testCallInfo) string {
	...
	b.WriteString("\tif err != nil {\n\t\tt.Fatal(err)\n\t}\n")
	b.WriteString("\t// a page of one from the first page, then the second, then the total is reached\n")
