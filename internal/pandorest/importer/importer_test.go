...
import (
	...
	"github.com/katbyte/radarr-mcp/internal/pandorest/config"
...
var miniConfig = config.Service{Name: "mini", Package: "mini", Naming: config.OperationIDNaming, TagSuffix: "Service", Auth: "Radarr"}
...
func TestImport(t *testing.T) {
	...
	t.Run("options", func(t *testing.T) {
		...
		if o.Pageable == nil || o.Pageable.PageOption != "Page" || o.Pageable.PageSizeOption != "PageSize" ||
			o.Pageable.ItemsField != "Records" || o.Pageable.TotalField != "TotalRecords" || o.Pageable.ItemType.String() != "Reference(Item)" {
	...
	t.Run("models", func(t *testing.T) {
		...
		if f := find(item.Fields, func(f definitions.Field) bool { return f.Name == "IsFolder" }); !f.Nullable || f.Required {
			t.Errorf("IsFolder is nullable %t, required %t; want nullable and not required", f.Nullable, f.Required)
		}
		if f := find(item.Fields, func(f definitions.Field) bool { return f.Name == "Id" }); !f.Required {
			t.Error("Id lost its required flag")
		}
...
// A NamePrefix is left out of the names of the paths under it, and only
// those: /api/v3/movie/{id} is GetMovieById, while /api, /api/v3alt and
// /ping keep every segment. The paths themselves keep the prefix.
func TestNamePrefix(t *testing.T) {
	t.Parallel()

	spec, err := openapi.Parse([]byte(`{
  "openapi": "3.0.4",
  "info": {"title": "Prefixed", "version": "1"},
  "paths": {
    "/api/v3/movie/{id}": {"get": {"tags": ["Movie"], "parameters": [{"name": "id", "in": "path", "schema": {"type": "integer"}}],
      "responses": {"200": {"content": {"application/json": {"schema": {"type": "string"}}}}}}},
    "/api/v3": {"get": {"tags": ["Api"], "responses": {"200": {"content": {"application/json": {"schema": {"type": "string"}}}}}}},
    "/api/v3alt": {"get": {"tags": ["Api"], "responses": {"200": {"content": {"application/json": {"schema": {"type": "string"}}}}}}},
    "/api": {"get": {"tags": ["Api"], "responses": {"200": {"content": {"application/json": {"schema": {"type": "string"}}}}}}},
    "/ping": {"get": {"tags": ["Ping"], "responses": {"200": {"content": {"application/json": {"schema": {"type": "string"}}}}}}}
  },
  "components": {"schemas": {}}
}`))
	if err != nil {
	...
	cfg := miniConfig
	cfg.Naming, cfg.NamePrefix, cfg.TagSuffix = config.PathNaming, "/api/v3", ""
	svc, err := FromSpec(cfg, spec, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	want := map[string]string{"/api/v3/movie/{id}": "GetMovieById", "/api/v3": "Get", "/api/v3alt": "GetApiV3alt", "/api": "GetApi", "/ping": "GetPing"}
	for path, name := range want {
		if got[path] != name {
			t.Errorf("%s is %q, want %q", path, got[path], name)
		}
	}
}
