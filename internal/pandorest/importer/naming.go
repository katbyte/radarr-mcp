...
// methodName is pathMethodName with the service's NamePrefix left out of
// the path: GET /api/v3/movie/{id} is GetMovieById. A path that only starts
// with the prefix's text (/api/v3alt) keeps it.
func (im *importer) methodName(method, path string) string {
	if p := im.cfg.NamePrefix; p != "" && (path == p || strings.HasPrefix(path, p+"/")) {
		path = strings.TrimPrefix(path, p)
	}

	return pathMethodName(method, path)
}
