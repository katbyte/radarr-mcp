...
func (im *importer) operation(namer *uniqueNamer, method, path string, op *openapi.Operation) definitions.Operation {
	want := im.methodName(method, path)
...
// jsonMedia reports whether a media type carries JSON. Swashbuckle lists
// text/json and application/*+json beside application/json, and other
// documents pair every JSON body with an application/xml twin.
...
func (im *importer) requestBody(method, path string, rb *openapi.RequestBody) *definitions.Body {
	...
	owner := im.methodName(method, path)
...
func (im *importer) responseBody(method, path string, op *openapi.Operation) *definitions.Body {
	for _, code := range openapi.SortedKeys(op.Responses) {
		...
		if other != "" || !hasJSON {
			// text/css alongside JSON is still a file, and so is XML on its
			// own; the text/plain Swashbuckle puts beside Radarr's JSON is
			// dropped by a workaround first
			if other == "" {
		...
		return &definitions.Body{ContentType: "application/json", Type: im.responseType(resp.Content[jsonType].Schema, im.methodName(method, path))}
	}
...
// markPageable flags the list operations that page with page and pageSize
// and answer a result with records and totalRecords (Radarr's
// PagingResource: history, the queue, the blocklist, the wanted lists and
// the log).
