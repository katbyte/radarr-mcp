// Command pandorest generates the Radarr SDK (lib/radarr) from Radarr's
// vendored OpenAPI document. It came from embyfin-mcp, which generates the
// Emby and Jellyfin SDKs the same way, and is inspired by Pandora, the
...
//	go run ./internal/pandorest generate -service radarr
//	go run ./internal/pandorest diff
//	go run ./internal/pandorest diff -old /tmp/old-definitions/radarr -new api-definitions/radarr
//
...
import (
	...
	"github.com/katbyte/radarr-mcp/internal/pandorest/differ"
...
func run(args []string, stdout, stderr io.Writer) error {
	...
	fs.SetOutput(stderr)
	services := fs.String("service", "", "comma-separated services to process (default all: radarr)")
