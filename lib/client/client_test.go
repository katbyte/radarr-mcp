...
// serve starts a canned server and a client for it, with the test's API key.
...
func TestNew(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ok", url: "http://nas:7878", auth: APIKey("t"), wantBase: "http://nas:7878"},
		{name: "trailing slash trimmed", url: "http://nas:7878/radarr/", auth: APIKey("t"), wantBase: "http://nas:7878/radarr"},
		{name: "missing url", url: "", auth: APIKey("t"), wantErr: "server URL is required"},
		{name: "no scheme", url: "nas:7878", auth: APIKey("t"), wantErr: "must include a scheme and host"},
		{name: "no host", url: "http://", auth: APIKey("t"), wantErr: "must include a scheme and host"},
		{name: "credentials in url", url: "http://kt:pw@nas:7878", auth: APIKey("t"), wantErr: "must not contain credentials"}, //nolint:gosec // the point of the case
		{name: "no authorizer", url: "http://nas:7878", wantErr: "authorizer is required"},
	}
...
func TestAuthorizer(t *testing.T) {
	...
	if v := got.Header.Get("X-Api-Key"); v != testToken {
		t.Errorf("X-Api-Key = %q, want %q", v, testToken)
	}
	// the key goes in the header only: Radarr also reads ?apikey=, which
	// would put it in every URL a proxy or a log records
	if got.URL.RawQuery != "" {
		t.Errorf("query = %q, want none", got.URL.RawQuery)
	}
	if got.Header.Get("Accept") != "application/json" || got.Header.Get("User-Agent") != UserAgent+"/"+version.Version {
		t.Errorf("Accept %q, User-Agent %q", got.Header.Get("Accept"), got.Header.Get("User-Agent"))
	}
}
...
func TestOptionsObject(t *testing.T) {
	...
	opts := options{
		query:  map[string][]string{"movieIds": {"1", "2"}, "pageSize": {"5"}},
		header: map[string]string{"X-Api-Key": "override"},
	}
	if _, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v3/history", ExpectedStatusCodes: []int{http.StatusOK}, OptionsObject: opts}, nil); err != nil {
		t.Fatal(err)
	}

	if got.URL.Path != "/api/v3/history" || got.URL.Query().Get("pageSize") != "5" {
		t.Errorf("request = %s", got.URL)
	}
	if ids := got.URL.Query()["movieIds"]; len(ids) != 2 || ids[0] != "1" || ids[1] != "2" {
		t.Errorf("movieIds = %v, want one key per value", ids)
	}
...
func TestStatusError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "403 has no hint", status: http.StatusForbidden, wantMsg: []string{"HTTP 403 (expected 200)"}},
...
func TestBodies(t *testing.T) {
	...
	t.Run("json both ways, body readable again", func(t *testing.T) {
		...
		c := serve(t, func(w http.ResponseWriter, r *http.Request) {
			...
			if string(b) != `{"label":"4k"}` || r.ContentLength != int64(len(b)) {
				t.Errorf("body = %s (ContentLength %d)", b, r.ContentLength)
			}
			w.WriteHeader(http.StatusCreated)
			_, _ = io.WriteString(w, `{"id":3,"label":"4k"}`)
		})
		resp, err := execute(t, c, RequestOptions{ContentType: "application/json", HttpMethod: http.MethodPost, Path: "/api/v3/tag", ExpectedStatusCodes: []int{http.StatusCreated}},
			struct {
		...
		if err := resp.Unmarshal(&model); err != nil || model.ID != 3 {
			t.Fatalf("Unmarshal = %+v, %v", model, err)
		}
		if b, _ := io.ReadAll(resp.Body); string(b) != `{"id":3,"label":"4k"}` {
	...
	t.Run("raw body needs a concrete type for a range", func(t *testing.T) {
		...
		req, err := c.NewRequest(t.Context(), RequestOptions{ContentType: "image/*", HttpMethod: http.MethodPost, Path: "/api/v3/mediacover/1/poster.jpg", ExpectedStatusCodes: []int{http.StatusNoContent}})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("stream left unread", func(t *testing.T) {
		...
		resp, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v3/log/file/radarr.txt", ExpectedStatusCodes: []int{http.StatusOK}, StreamResponse: true}, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestListHelpers(t *testing.T) {
	...
	type kind string
	if got := CSV([]kind{"grabbed", "downloadFolderImported"}); got != "grabbed,downloadFolderImported" {
