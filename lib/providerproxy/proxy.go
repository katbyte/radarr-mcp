...
// service Radarr calls out to. It came from embyfin-mcp, where it stands in
// front of Emby and Jellyfin the same way.
//
// radarr-mcp never talks to Radarr's metadata service (api.radarr.video, its
// front for TMDB) or the image CDN itself: it asks Radarr to (look a film up,
// add it, refresh it), and Radarr - a .NET app in a container - makes those
...
type Options struct {
	...
	// next, and a version or architecture from one machine to the next, and
	// none of them may decide whether a cassette matches, nor be committed
	// with it.
	RedactQuery []string
	// RedactBodyFields names JSON fields whose string value is replaced in a
	// recorded response body: a credential a provider answers with is not
	// something the repository may carry, and replay never needs one, because
	// the proxy answers the calls it would authorise.
	RedactBodyFields []string
	// IgnoreHosts are hosts this proxy answers 204 for and never records:
	// the server talking to itself, or to a service whose answers are no
	// part of what these cassettes are about. Radarr checks its clock and its
	// news against radarr.servarr.com with its version and architecture in
	// the query, and a recorded clock is wrong a day later.
	IgnoreHosts []string
}
...
func newCA() (*x509.Certificate, *ecdsa.PrivateKey, error) {
	...
	tmpl := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "radarr-mcp provider proxy CA"},
