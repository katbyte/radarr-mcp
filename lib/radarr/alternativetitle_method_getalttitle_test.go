...
func TestOperationGetAlttitle(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetAlttitle(t.Context(), GetAlttitleOperationOptions{
		MovieId:         7,
		MovieMetadataId: 7,
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/alttitle")
	expectQuery(t, r, "movieId", "7")
	expectQuery(t, r, "movieMetadataId", "7")
