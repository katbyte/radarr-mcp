...
func TestOperationGetHistoryMovie(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetHistoryMovie(t.Context(), GetHistoryMovieOperationOptions{
		MovieId:      7,
		EventType:    MovieHistoryEventTypeUnknown,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/history/movie")
	expectQuery(t, r, "movieId", "7")
