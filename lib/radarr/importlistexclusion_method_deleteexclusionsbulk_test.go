...
func TestOperationDeleteExclusionsBulk(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteExclusionsBulk(t.Context(), ImportListExclusionBulkResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/api/v3/exclusions/bulk")
