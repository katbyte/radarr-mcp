...
func TestOperationPostExclusions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 201, "application/json", "{}")
	result, err := c.PostExclusions(t.Context(), ImportListExclusionResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v3/exclusions")
