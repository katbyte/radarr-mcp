...
func TestOperationPostExclusionsBulk(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.PostExclusionsBulk(t.Context(), []ImportListExclusionResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v3/exclusions/bulk")
