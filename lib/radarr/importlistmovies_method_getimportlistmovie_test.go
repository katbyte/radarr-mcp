...
func TestOperationGetImportlistMovie(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetImportlistMovie(t.Context(), GetImportlistMovieOperationOptions{
		IncludeRecommendations: new(true),
		IncludeTrending:        new(true),
		IncludePopular:         new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/importlist/movie")
	expectQuery(t, r, "includeRecommendations", "true")
	expectQuery(t, r, "includeTrending", "true")
	expectQuery(t, r, "includePopular", "true")
