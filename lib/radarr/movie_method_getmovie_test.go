...
func TestOperationGetMovie(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetMovie(t.Context(), GetMovieOperationOptions{
		TmdbId:             7,
		ExcludeLocalCovers: new(true),
		LanguageId:         7,
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/movie")
	expectQuery(t, r, "tmdbId", "7")
	expectQuery(t, r, "excludeLocalCovers", "true")
	expectQuery(t, r, "languageId", "7")
