...
func TestOperationPutMovieById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutMovieById(t.Context(), 7, MovieResource{}, PutMovieByIdOperationOptions{
		MoveFiles: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v3/movie/7")
