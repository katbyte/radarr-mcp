...
func TestOperationGetMoviefile(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetMoviefile(t.Context(), GetMoviefileOperationOptions{
		MovieId:      []int{1, 2},
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/moviefile")
	expectQuery(t, r, "movieId", "1", "2")
	expectQuery(t, r, "movieFileIds", "1", "2")
