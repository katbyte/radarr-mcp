...
func TestOperationGetMovieLookup(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetMovieLookup(t.Context(), GetMovieLookupOperationOptions{
		Term: "v-Term",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/movie/lookup")
