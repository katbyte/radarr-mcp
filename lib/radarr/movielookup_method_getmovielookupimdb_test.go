...
func TestOperationGetMovieLookupImdb(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetMovieLookupImdb(t.Context(), GetMovieLookupImdbOperationOptions{
		ImdbId: "v-ImdbId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/movie/lookup/imdb")
