...
curl -sSfL "https://github.com/katbyte/radarr-mcp/archive/refs/tags/${TAG}.tar.gz" -o "$tmp/src.tar.gz"
SHA256="$(sha256sum "$tmp/src.tar.gz" | cut -d' ' -f1)"

cat > "$tmp/radarr-mcp.rb" <<FORMULA
# typed: false
...
class RadarrMcp < Formula
  ...
  url "https://github.com/katbyte/radarr-mcp/archive/refs/tags/${TAG}.tar.gz"
  sha256 "${SHA256}"
  ...
  test do
    assert_match "${TAG}", shell_output("#{bin}/radarr-mcp version")
  end
end
FORMULA
...
cp "$tmp/radarr-mcp.rb" "$tmp/tap/Formula/radarr-mcp.rb"
cd "$tmp/tap"
git config user.name "radarr-mcp-release"
git config user.email "radarr-mcp-release@katbyte.me"
git add Formula/radarr-mcp.rb
git diff --cached --quiet && { echo "==> formula unchanged"; exit 0; }
git commit -m "radarr-mcp ${TAG}"
git push
echo "==> done"
